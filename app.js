...
class CrystalApp {
    ...
    async init() {
        ...
        if (!this.testStorage()) {
            console.error('Cannot proceed - storage is disabled');
            return;
        }
    ...
    loadData() {
        try {
            ...
            if (savedCrystals) {
                ...
                console.log('Successfully loaded crystals:', this.crystals);
            } else {
                console.log('No saved crystals found in localStorage');
            }
        } catch (error) {
            console.error('Error loading crystals from localStorage:', error);
            this.showToast('Error loading saved data. Storage may be disabled.');
            this.crystals = [];
        }
        ...
        try {
            ...
            if (savedSettings) {
                ...
                // Update settings UI
                const autoCheck = document.getElementById('autoCheckLocation');
                const radius = document.getElementById('notificationRadius');
                const apiKey = document.getElementById('apiKey');
                
                if (autoCheck) autoCheck.checked = this.settings.autoCheckLocation;
                if (radius) radius.value = this.settings.notificationRadius;
    ...
    saveData() {
        try {
            ...
            if (typeof(Storage) === "undefined") {
                ...
                this.showToast('⚠️ Storage not available. Data will not be saved.');
                return false;
            }

            // Try to save
            const crystalsData = JSON.stringify(this.crystals);
            const settingsData = JSON.stringify(this.settings);
            
            localStorage.setItem('crystals', crystalsData);
            localStorage.setItem('settings', settingsData);
            
            console.log(`Saved ${this.crystals.length} crystals to localStorage`);
            
            // Verify save worked
            const verify = localStorage.getItem('crystals');
            if (verify !== crystalsData) {
                console.error('Save verification failed!');
                this.showToast('⚠️ Data may not have saved correctly');
                return false;
            }
            ...
            if (error.name === 'QuotaExceededError') {
                this.showToast('⚠️ Storage full! Try deleting old photos.');
            } else {
                this.showToast('⚠️ Error saving data. Check Safari settings.');
            }
            return false;
        }
    }
    ...
    testStorage() {
        try {
            ...
            alert('Storage is disabled. Please enable cookies/storage in Safari settings:\nSettings → Safari → Disable "Block All Cookies"');
            return false;
        }
    }
    ...
    async saveCrystal(event) {
        ...
        if (saved) {
            console.log('Crystal saved successfully. Total crystals:', this.crystals.length);
        } else {
            console.error('Failed to save crystal!');
            alert('⚠️ Crystal was not saved! Check Safari settings: Settings → Safari → Disable "Block All Cookies"');
        }
